...
def testimonial_card(idx, comment, name, role, image_src):
    return Div(
...
def stacked_card(title, description, stacks, color):
    return Div(
        Div(
            ...
            Div(*starmap(stack_item, stacks), cls=f"{gap2} flex-wrap items-center"),
        ...
        Div(
            Div(
                ...
                Div(
                    ...
                    # 'Close button',
                    Button(
                        "X", id="closePopup", cls="text-green-800 hover:text-green-950"
                    ),
...
def video_button(
    ...
    video_duration,
    video_id,
    poster_alt="Video poster",
    youtube_icon_src="/assets/icons/youtube.svg",
    max_width="350px",
):
    return (
        A(
            Img(
                ...
                width="240",
                height="120",
                cls="rounded-full w-[7.5rem] h-auto",
                alt=poster_alt,
            ),
            Span(
                txt,
                Span(video_duration, cls="s-body text-black/60"),
        ...
        Div(
            ...
            Button(
                ...
                id="closeVideo",
                cls="mt-4 bg-soft-pink text-black font-bold py-2 px-4 rounded",
            ),
            Script(
                "document.addEventListener('keydown', (e) => { if (e.key === 'Escape') me('#closeVideo').send('click'); });"
            ),
            id="videoOverlay",
            cls="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50",
        ),
    )
